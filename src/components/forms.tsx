...
export function DeathDialog({ players, disabled }: { players: Player[]; disabled?: boolean }) {
  ...
  return <>
    ...
    <dialog ref={dialog} className="m-auto max-h-[92vh] w-[min(94vw,620px)] overflow-y-auto rounded-3xl border border-[#343f37] bg-[#0e120f] p-0 text-white shadow-2xl">
      <div className="flex items-start justify-between border-b border-[#242d27] p-5 sm:p-6"><div><p className="eyebrow mb-2 text-[#ff7378]">Fin del intento</p><h2 className="text-2xl font-black">☠️ Registrar muerte</h2></div><button onClick={()=>dialog.current?.close()} className="grid size-10 place-items-center rounded-xl bg-[#171d19]" aria-label="Cerrar"><X size={18}/></button></div>
